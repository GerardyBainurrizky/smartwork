@if ($paginator->total() > 0)
    <nav role="navigation" aria-label="Pagination" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 w-full">
        ...
        <div class="flex flex-col gap-2 w-full sm:hidden">
            ...
            <div class="flex items-center justify-between w-full gap-2">
                @if ($paginator->onFirstPage())
                    <span class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-gray-400 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl cursor-default select-none">
                ...
                @else
                    <a href="{{ $paginator->previousPageUrl() }}" rel="prev"
                        class="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold text-[#0DA4CE] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm hover:bg-[#0DA4CE]/5 hover:border-[#0DA4CE]/40 transition">
                        <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                        Sebelumnya
                    </a>
                @endif
        ...
        <div class="hidden sm:flex sm:items-center gap-1.5 flex-wrap justify-end">
            @if ($paginator->onFirstPage())
                <span class="inline-flex items-center justify-center px-3 h-9 text-sm font-medium text-gray-400 bg-gray-50 border border-gray-200 rounded-lg cursor-default select-none">
                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                    Sebelumnya
                </span>
            @else
                <a href="{{ $paginator->previousPageUrl() }}" rel="prev"
                    class="inline-flex items-center justify-center px-3 h-9 text-sm font-semibold text-[#0DA4CE] bg-white border border-gray-200 rounded-lg hover:bg-[#0DA4CE]/5 hover:border-[#0DA4CE]/40 transition">
                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                    Sebelumnya
                </a>
            @endif
            ...
            @foreach ($elements as $element)
                @if (is_string($element))
                    <span class="px-1.5 text-sm text-gray-400">{{ $element }}</span>
                @endif
