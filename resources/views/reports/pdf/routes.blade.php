...
{{-- Extended metadata for Sales Routes --}}
...
@foreach($items as $index => $route)
    @php
        $estimated = $route->stops->sum('estimated_duration_minutes');
    ...
    <div class="section">
        <div class="section-title">
            Rencana Kunjungan #{{ $index + 1 }} &mdash; {{ $route->name }}
        ...
        <div class="section-body">
            <table class="detail">
                <tr>
                    ...
                    <td class="k">Tanggal Route</td>
                    <td class="v">{{ $route->date->format('d M Y') }}</td>
                </tr>
                <tr>
                    <td class="k">Rute Wilayah</td>
                    <td class="v">{{ $route->name }}</td>
                    <td class="k">Status Route</td>
                    <td class="v">{{ $label }}</td>
                </tr>
                <tr>
                    <td class="k">Jumlah Toko</td>
                    <td class="v">{{ $route->stops->count() }} toko</td>
                    <td class="k">Toko Dikunjungi</td>
                    <td class="v">{{ $route->stops->where('status', 'visited')->count() }} toko</td>
                </tr>
                <tr>
                    <td class="k">Estimasi Durasi</td>
                    <td class="v">{{ $estimated > 0 ? $estimated . ' menit' : '-' }}</td>
                    <td class="k">Durasi Aktual</td>
                    <td class="v">{{ $actual }}</td>
                </tr>
                <tr>
                    <td class="k">Dibuat Oleh</td>
                    <td class="v">{{ $route->creator->name ?? ($route->created_by_label ?? '-') }}</td>
            ...
            <table class="data" style="margin-top:8px;">
                <thead>
                    <tr>
                        ...
                        <th style="width:11%">Status Toko</th>
                        <th style="width:23%">Catatan Toko</th>
                ...
                <tbody>
                    @foreach($route->stops as $sIndex => $stop)
                        ...
                        <tr>
                            ...
                            <td>{{ $store->name ?? '-' }}</td>
                            <td>{{ $store->address ?? '-' }}</td>
                            <td>{{ $stop->estimated_duration_minutes ? $stop->estimated_duration_minutes . ' mnt' : '-' }}</td>
                            <td>
...
@if($items->isEmpty())
    <div class="empty">Tidak ada data rencana kunjungan pada periode ini.</div>
@endif
