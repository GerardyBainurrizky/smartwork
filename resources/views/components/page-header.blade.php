@props(['title' => '', 'subtitle' => '', 'icon' => 'default', 'accent' => '#0DA4CE'])

<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div class="flex items-center gap-3.5 min-w-0">
        <div class="w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center shrink-0" style="background: {{ $accent }}14; color: {{ $accent }};">
            @if($icon === 'route')
        ...
        <div class="min-w-0">
            <h1 class="text-lg sm:text-xl font-extrabold text-gray-900 dark:text-white tracking-tight leading-tight">{{ $title }}</h1>
            @if($subtitle)
