<x-app-layout>
    ...
    <div class="animate-fade-in space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="sw-card p-5">
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs text-gray-500 dark:text-gray-400">Rencana Kunjungan Bulan Ini</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $plannedVisitCount }}</p>
                    </div>
                </div>
            </div>
            <div class="sw-card p-5">
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs text-gray-500 dark:text-gray-400">Kunjungan Hari Ini</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $todayVisitStats['total'] }}</p>
                    </div>
                </div>
            </div>
            <div class="sw-card p-5">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-[#097A99]/10 flex items-center justify-center shrink-0">
                        <svg class="w-5 h-5 text-[#097A99]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs text-gray-500 dark:text-gray-400">Kunjungan Selesai</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $todayVisitStats['completed'] }}</p>
                    </div>
                </div>
            </div>
        </div>

        {{-- Ringkasan Transaksi & Piutang Sales --}}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            {{-- Card 1: Aktivitas Transaksi Hari Ini (Total Transaksi Baru & Uang Masuk Transaksi Baru) --}}
            <div class="sw-card p-5 flex flex-col justify-between">
                <div>
                    <div class="flex items-center justify-between gap-2">
                        <span class="text-xs font-semibold text-gray-500 dark:text-gray-400">Aktivitas Transaksi Hari Ini</span>
                    ...
                    <div class="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 border-b border-gray-100 dark:border-gray-800 pb-3">
                        <div>
                            <p class="text-[11px] text-gray-400 dark:text-gray-500 uppercase font-semibold leading-tight">Total Transaksi Baru</p>
                            <p class="text-base sm:text-lg font-bold font-mono text-gray-900 dark:text-white leading-tight break-words mt-1">Rp {{ number_format($todayVisitStats['transaction'], 0, ',', '.') }}</p>
                            <p class="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 leading-tight">Nilai seluruh transaksi baru hari ini.</p>
                        </div>
                        <div>
                            <p class="text-[11px] text-emerald-600 dark:text-emerald-400 uppercase font-semibold leading-tight">Uang Masuk Transaksi Baru</p>
                            <p class="text-base sm:text-lg font-bold font-mono text-emerald-600 dark:text-emerald-400 leading-tight break-words mt-1">Rp {{ number_format($todayVisitStats['cash_in'] ?? 0, 0, ',', '.') }}</p>
                            <p class="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 leading-tight">Uang diterima untuk transaksi baru hari ini.</p>
                        </div>
                    </div>
                </div>
                <p class="text-[11px] text-gray-400 dark:text-gray-500 mt-2">Penjualan baru &amp; pembayaran langsung transaksi hari ini.</p>
            </div>

            {{-- Card 2: Total Saldo Piutang --}}
            <div class="sw-card p-5 flex flex-col justify-between">
                <div>
                    <div class="flex items-center justify-between gap-2">
                        <span class="text-xs font-semibold text-gray-500 dark:text-gray-400">Total Saldo Piutang</span>
                        <div class="w-10 h-10 rounded-xl bg-red-500/10 flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                        </div>
                    </div>
                    <p class="mt-3 text-lg sm:text-2xl font-bold font-mono text-red-600 dark:text-red-400 leading-tight break-words">Rp {{ number_format($todayVisitStats['current_receivable'], 0, ',', '.') }}</p>
                </div>
                <p class="text-[11px] text-gray-400 dark:text-gray-500 mt-2">Seluruh saldo piutang toko yang masih belum lunas.</p>
            </div>

            {{-- Card 3: Pembayaran Piutang Lama Hari Ini --}}
            <div class="sw-card p-5 flex flex-col justify-between">
                <div>
                    <div class="flex items-center justify-between gap-2">
                        <span class="text-xs font-semibold text-gray-500 dark:text-gray-400">Pembayaran Piutang Lama Hari Ini</span>
                    ...
                    <p class="mt-3 text-lg sm:text-2xl font-bold font-mono text-emerald-600 dark:text-emerald-400 leading-tight break-words">Rp {{ number_format($todayVisitStats['debt_payment_today'], 0, ',', '.') }}</p>
                </div>
                <p class="text-[11px] text-gray-400 dark:text-gray-500 mt-2">Uang yang diterima untuk pembayaran piutang yang sudah ada sebelumnya.</p>
            </div>
        </div>

        {{-- Jadwal Kunjungan Hari Ini + Ringkasan Aktivitas --}}
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div class="sw-card overflow-hidden">
                ...
                <div class="p-3 divide-y divide-gray-50 dark:divide-gray-800">
                    @if($todayStops->count() > 0)
                        ...
                        <div class="flex items-center gap-3 p-2.5">
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium shrink-0
                                ...
                                @if($stop->computed_status === 'visited') Dikunjungi
                    ...
                    <div class="text-center py-8 px-4">
                        <p class="text-sm text-gray-500 dark:text-gray-400">Belum ada rencana kunjungan untuk hari ini.</p>
                        <a href="{{ route('route.create') }}" class="inline-flex items-center mt-3 px-4 py-2 bg-[#0DA4CE] text-white rounded-lg text-xs font-semibold hover:bg-[#097A99] transition">Buat Rencana Kunjungan</a>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="sw-card overflow-hidden">
                ...
                <div class="divide-y divide-gray-50 dark:divide-gray-800">
                    ...
                    @empty
                    <div class="p-8 text-center text-sm text-gray-500 dark:text-gray-400">Belum ada kunjungan hari ini.</div>
                    @endforelse
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="sw-card p-5">
                <p class="text-xs text-gray-500 dark:text-gray-400">Kunjungan Bulan Ini</p>
                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ $monthlyStats['visits'] }}</p>
            </div>
            <div class="sw-card p-5">
                <p class="text-xs text-gray-500 dark:text-gray-400">Total Transaksi Baru Bulan Ini</p>
                <p class="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white mt-1 leading-tight break-words">Rp {{ number_format($monthlyStats['transaction'], 0, ',', '.') }}</p>
            </div>
            <div class="sw-card p-5">
                <p class="text-xs text-gray-500 dark:text-gray-400">Tingkat Penyelesaian Bulan Ini</p>
                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ $monthlyStats['completion_rate'] ?? 0 }}%</p>
            </div>
        </div>
    </div>
