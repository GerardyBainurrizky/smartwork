<x-app-layout>
    <x-slot name="header">
         <x-page-header icon="route" title="Rencana Kunjungan" subtitle="Buat, pantau, dan kelola seluruh rute kunjungan Sales">
    ...
    <div class="max-w-7xl mx-auto" x-data="adminRouteDelete()">
        ...
        <div id="sw-route-filter-card" class="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-4 mb-6">
            <form method="GET" action="{{ route('admin.routes.index') }}" class="w-full grid grid-cols-2 gap-3 lg:flex lg:flex-row lg:flex-wrap lg:items-end lg:gap-3">
                ...
                <div class="sw-route-field flex flex-col items-stretch gap-2 w-full min-w-0 pt-5 lg:pt-0 lg:w-auto lg:flex-row lg:items-center lg:gap-3">
                    <button type="submit"
                        class="inline-flex items-center justify-center w-full lg:w-auto px-5 py-3 lg:py-2.5 bg-[#0DA4CE] text-white rounded-xl text-sm font-semibold hover:bg-[#097A99] transition shadow-sm shadow-[#0DA4CE]/20">
                    ...
                    <a href="{{ route('admin.routes.index') }}"
                        class="inline-flex items-center justify-center w-full lg:w-auto px-5 py-3 lg:py-2.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-xl text-sm font-semibold hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-4">
                    <div class="flex items-start justify-between gap-3 mb-3">
                        <div class="min-w-0">
                            <div class="flex items-center gap-2 mb-1">
                                <div class="w-6 h-6 rounded-full bg-[#0DA4CE]/10 flex items-center justify-center text-[10px] font-bold text-[#0DA4CE] shrink-0">
                                    {{ strtoupper(substr($route->user->name ?? 'S', 0, 2)) }}
                                </div>
                                <p class="text-xs text-gray-400 truncate">{{ $route->user->name ?? 'Sales' }}</p>
                            </div>
                        ...
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium shrink-0
                            @if($route->computed_status === 'draft') bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300
                            @elseif($route->computed_status === 'active') bg-[#0DA4CE]/10 text-[#0DA4CE]
                            @elseif($route->computed_status === 'completed') bg-[#90F022]/20 text-green-800 dark:text-green-300
...
@endif
                    ...
                    <div class="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400 mb-3">
                        ...
                        <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400">
                            Dibuat oleh {{ $route->created_by_label }}
                        </span>
                    </div>
                    ...
                    <div class="mb-3">
                        <div class="flex flex-wrap gap-2 text-xs">
                            ...
                            <span class="inline-flex items-center gap-1 text-[#0DA4CE]">
                    ...
                    @if(($route->stops ?? collect())->isNotEmpty())
                    <div class="flex flex-wrap gap-1 mb-3">
                        @foreach($route->stops->take(3) as $stop)
                        <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400">
                            {{ $stop->store->name ?? 'Toko' }}
                        </span>
                        @endforeach
                        @if(($route->stops_count ?? 0) > 3)
                        <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs bg-gray-100 dark:bg-gray-800 text-gray-400">
                            +{{ ($route->stops_count ?? 0) - 3 }}
                        </span>
                        @endif
                    </div>
                    @endif
                    ...
                    <a href="{{ route('route.show', $route->id) }}"
                        class="inline-flex items-center justify-center w-full px-4 py-2 bg-[#0DA4CE]/10 text-[#0DA4CE] rounded-xl text-sm font-semibold hover:bg-[#0DA4CE]/20 transition">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('admin.routes.edit', $route->id) }}"
                            class="inline-flex items-center justify-center flex-1 px-4 py-2 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-xl text-sm font-semibold hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                            Edit
                        </a>
                        <button type="button" @click="openDelete({
                            name: @js($route->name),
                            url: @js(route('admin.routes.destroy', $route->id))
                        })"
                            class="inline-flex items-center justify-center px-4 py-2 bg-red-50 dark:bg-red-900/40 text-red-600 dark:text-red-400 rounded-xl text-sm font-semibold hover:bg-red-100 dark:hover:bg-red-900 transition">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-12 text-center">
                ...
                <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">Tidak ada rute yang ditemukan</p>
            </div>
            @endforelse
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-800">
                    <thead class="bg-gray-50 dark:bg-gray-800/50">
                        <tr>
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Sales</th>
                            ...
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Toko</th>
                            ...
                            <th class="px-4 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Dibuat Oleh</th>
                        ...
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/40 transition">
                            <td class="px-4 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $route->user->name ?? '-' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-4">
                                <a href="{{ route('route.show', $route->id) }}" class="text-sm font-medium text-[#0DA4CE] hover:text-[#097A99] transition">
                                    {{ $route->name }}
                                </a>
                            </td>
                            ...
                            <td class="px-4 py-4">
                                ...
                                <div class="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">
                                    <span class="text-green-700 dark:text-green-400">{{ $p['selesai'] }} selesai</span> &middot;
                                    <span class="text-[#0DA4CE]">{{ $p['berjalan'] }} berjalan</span> &middot;
                                    <span>{{ $p['belum'] }} belum</span>
                                    @if(($p['dilewati'] ?? 0) > 0) &middot; <span class="text-red-600 dark:text-red-400">{{ $p['dilewati'] }} dilewati</span>@endif
                                </div>
                                <div class="mt-1.5 h-1.5 w-32 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                                    <div class="h-full bg-[#90F022] rounded-full transition-all" style="width: {{ round((($p['selesai'] + $p['dilewati']) / max($p['total'], 1)) * 100) }}%"></div>
                                </div>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-4 whitespace-nowrap text-right">
                                <div class="inline-flex items-center gap-2">
                                    ...
                                    @if(in_array($route->status, ['draft', 'cancelled']))
                                    <a href="{{ route('admin.routes.edit', $route->id) }}"
                                        class="inline-flex items-center px-3 py-1.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-lg text-xs font-semibold hover:bg-gray-200 dark:hover:bg-gray-700 transition">Edit</a>
                                    <button type="button" @click="openDelete({
                                        name: @js($route->name),
                                        url: @js(route('admin.routes.destroy', $route->id))
                                    })"
                                        class="inline-flex items-center px-3 py-1.5 bg-red-50 dark:bg-red-900/40 text-red-600 dark:text-red-400 rounded-lg text-xs font-semibold hover:bg-red-100 dark:hover:bg-red-900 transition">
        ...
        <div class="mt-8">
            <div class="flex items-center gap-2 mb-4">
                <svg class="w-5 h-5 text-[#0DA4CE]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Rute Mendatang</h2>
                @if(($upcomingRoutes ?? collect())->isNotEmpty())
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#0DA4CE]/10 text-[#0DA4CE]">{{ $upcomingRoutes->count() }}</span>
                @endif
            </div>

            @forelse($upcomingRoutes ?? [] as $route)
            @php $p = $route->progress ?? ['total' => 0, 'selesai' => 0, 'berjalan' => 0, 'belum' => 0, 'dilewati' => 0]; @endphp
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-4 mb-3">
                <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                    <div class="flex items-center gap-3 flex-1 min-w-0">
                        <div class="w-10 h-10 rounded-full bg-[#0DA4CE]/10 flex items-center justify-center text-xs font-bold text-[#0DA4CE] shrink-0">
                            {{ strtoupper(substr($route->user->name ?? 'S', 0, 2)) }}
                        </div>
                        <div class="min-w-0">
                            <a href="{{ route('route.show', $route->id) }}" class="font-semibold text-gray-900 dark:text-white hover:text-[#0DA4CE] transition truncate block">
                                {{ $route->name }}
                            </a>
                            <p class="text-xs text-gray-500 dark:text-gray-400 truncate">Sales: {{ $route->user->name ?? '-' }}</p>
                        </div>
                    </div>
            ...
            @empty
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-10 text-center">
                <p class="text-sm text-gray-500 dark:text-gray-400">Tidak ada rute mendatang</p>
            </div>
            @endforelse
        </div>

        {{-- Modal Konfirmasi Hapus Rencana Kunjungan --}}
                ...
                <div class="p-6">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">Hapus Rencana Kunjungan</h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Apakah Anda yakin ingin menghapus rencana kunjungan ini? Seluruh data rencana kunjungan akan dihapus dari Admin dan Sales yang dituju.</p>
                        </div>
                ...
                async confirmDelete() {
                    ...
                    try {
                        ...
                        if (res.ok && data.status === 'success') {
                            window.showToast(data.message || 'Rencana kunjungan berhasil dihapus.', 'success');
                            this.deleteRoute = null;
                            setTimeout(() => window.location.reload(), 600);
                        } else {
                            window.showToast(data.message || 'Terjadi kesalahan saat menghapus rencana kunjungan.', 'error');
                            this.deleteRoute = null;
                        }
                    } catch (e) {
                        window.showToast('Terjadi kesalahan saat menghapus rencana kunjungan.', 'error');
                        this.deleteRoute = null;
                    } finally {
                        this.deleting = false;
                    }
