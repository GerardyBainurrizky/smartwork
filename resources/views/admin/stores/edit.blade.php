<x-app-layout>
    <x-slot name="header">
        <x-page-header icon="store" title="Edit Toko" subtitle="Perbarui data master toko dan jangkauan Sales"></x-page-header>
    </x-slot>

    <div class="max-w-2xl mx-auto">
        <a href="{{ route('admin.stores.index') }}" class="inline-flex items-center text-sm text-gray-500 hover:text-[#0DA4CE] mb-4 transition">
        ...
        <div class="sw-card bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden"
             x-data="{
                 salesId: '{{ old('sales_penanggung_jawab_id', $store->sales_penanggung_jawab_id ?? '') }}',
                 delivery: '{{ old('is_delivery_destination', $store->is_delivery_destination ? '1' : '0') }}',
                 get hasSales() {
            ...
            <div class="p-6">
                <div class="flex items-center justify-between mb-6">
                    <h1 class="text-xl font-bold text-gray-900 dark:text-white">Edit Toko</h1>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300">{{ $store->code }}</span>
                </div>

                <form action="{{ route('admin.stores.update', $store->id) }}" method="POST" class="space-y-5">
                    @csrf @method('PUT')

                    <div>
                        <label class="sw-label" for="name">Nama Toko</label>
                        <input type="text" id="name" name="name" value="{{ old('name', $store->name) }}"
                            class="sw-input {{ $errors->has('name') ? 'sw-input-error' : '' }}" required>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" id="owner" name="owner" value="{{ old('owner', $store->owner) }}"
                                class="sw-input {{ $errors->has('owner') ? 'sw-input-error' : '' }}">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                        <div>
                            ...
                            <input type="text" id="city" name="city" value="{{ old('city', $store->city) }}"
                                class="sw-input {{ $errors->has('city') ? 'sw-input-error' : '' }}" required>
                        ...
                        <div>
                            ...
                            <input type="text" id="kecamatan" name="kecamatan" value="{{ old('kecamatan', $store->kecamatan) }}"
                                class="sw-input {{ $errors->has('kecamatan') ? 'sw-input-error' : '' }}">
                        ...
                        <div>
                            ...
                            <input type="text" id="province" name="province" value="{{ old('province', $store->province) }}"
                                class="sw-input {{ $errors->has('province') ? 'sw-input-error' : '' }}" required>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" id="latitude" name="latitude" value="{{ old('latitude', $store->latitude) }}" inputmode="decimal"
                        ...
                        <div>
                            ...
                            <input type="text" id="longitude" name="longitude" value="{{ old('longitude', $store->longitude) }}" inputmode="decimal"
                    ...
                    <div>
                        ...
                        <input type="url" id="maps_url" name="maps_url" value="{{ old('maps_url', $store->maps_url) }}"
                    ...
                    <div>
                        ...
                        <div class="mt-2 flex gap-6">
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" name="status" value="active" class="text-[#0DA4CE] focus:ring-[#0DA4CE]" {{ old('status', $store->status) === 'active' ? 'checked' : '' }}>
                                <span class="ml-2 text-sm text-gray-700 dark:text-gray-300 font-medium">Aktif</span>
                            </label>
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" name="status" value="inactive" class="text-[#0DA4CE] focus:ring-[#0DA4CE]" {{ old('status', $store->status) === 'inactive' ? 'checked' : '' }}>
