<x-app-layout>
    @section('breadcrumbs')
        ...
        <span>Pembayaran Piutang Hari Ini</span>
    @endsection

    <x-slot name="header">
        <x-page-header icon="clock" title="Pembayaran Piutang Hari Ini" subtitle="Daftar penerimaan pelunasan piutang lama yang diterima khusus pada hari ini"></x-page-header>
    </x-slot>
    ...
    <div class="space-y-6 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="sw-card p-5 bg-gradient-to-br from-emerald-50/80 to-white dark:from-emerald-950/25 dark:to-gray-800 border-emerald-100 dark:border-emerald-900/40">
                <span class="text-[11px] font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-400">Total Uang Masuk Hari Ini</span>
                <p class="text-2xl sm:text-3xl font-extrabold font-mono text-emerald-600 dark:text-emerald-400 mt-2">
                    Rp {{ number_format($totalAmount, 0, ',', '.') }}
                </p>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Tanggal: {{ \Carbon\Carbon::parse($todayDate)->format('d M Y') }}</p>
            </div>
            <div class="sw-card p-5">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Pembayaran piutang lama yang tercatat hari ini</p>
            </div>
        </div>

        {{-- Filter & Search Box --}}
        <div class="sw-card p-4 sm:p-5">
            <form method="GET" action="{{ route('admin.receivables.summary.today-payments') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 items-end">
                <div>
                ...
                <div class="flex items-center pb-2">
                    <a href="{{ route('admin.receivables.summary.today-payments') }}" class="text-xs text-red-600 hover:underline font-semibold">Reset Filter</a>
                </div>
                @endif
            </form>
        </div>

        {{-- Table Pembayaran Hari Ini --}}
        <div class="sw-card overflow-hidden">
            <div class="p-4 sm:p-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <div>
                    <h3 class="font-bold text-gray-900 dark:text-gray-100 text-sm sm:text-base">Daftar Pembayaran Hari Ini</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Rincian uang kas yang masuk untuk piutang lama hari ini</p>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-800/60 text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">
                        <tr>
                            <th class="px-5 py-3 font-semibold text-left">Waktu Dicatat</th>
