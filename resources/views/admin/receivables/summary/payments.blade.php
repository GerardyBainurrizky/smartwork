<x-app-layout>
    ...
    <x-slot name="header">
        <x-page-header icon="history" title="Riwayat Pembayaran Piutang" subtitle="Histori lengkap seluruh pencatatan pembayaran untuk pelunasan piutang lama"></x-page-header>
    </x-slot>
    ...
    <div class="space-y-6 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="sw-card p-5 bg-gradient-to-br from-green-50/80 to-white dark:from-green-950/25 dark:to-gray-800 border-green-100 dark:border-green-900/40">
                <span class="text-[11px] font-bold uppercase tracking-wider text-green-700 dark:text-green-400">Total Akumulasi Pembayaran</span>
                <p class="text-2xl sm:text-3xl font-extrabold font-mono text-green-600 dark:text-green-400 mt-2">
                    Rp {{ number_format($totalAmount, 0, ',', '.') }}
                </p>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Total seluruh pembayaran piutang lama yang tercatat</p>
            </div>
            <div class="sw-card p-5">
                <span class="text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Total Entri Pembayaran</span>
                <p class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100 mt-2">
                    {{ $payments->total() }} <span class="text-sm font-normal text-gray-400">transaksi pembayaran</span>
                </p>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Semua periode &amp; metode</p>
            </div>
        </div>

        {{-- Filter & Search Box --}}
        <div class="sw-card p-4 sm:p-5">
            <form method="GET" action="{{ route('admin.receivables.summary.payments') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
                <div>
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Cari Toko / Transaksi</label>
                ...
                @if(request()->hasAny(['sales_id', 'payment_method', 'search', 'from_date', 'to_date']))
                <div class="flex items-center pb-2">
                    <a href="{{ route('admin.receivables.summary.payments') }}" class="text-xs text-red-600 hover:underline font-semibold">Reset Filter</a>
                </div>
                @endif
            </form>
        </div>
        ...
        <div class="sw-card overflow-hidden">
            <div class="p-4 sm:p-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <div>
                    <h3 class="font-bold text-gray-900 dark:text-gray-100 text-sm sm:text-base">Daftar Pembayaran Piutang Lama</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Seluruh penerimaan kas pelunasan piutang</p>
                </div>
                ...
                <table class="w-full text-left text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-800/60 text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">
                        <tr>
                            ...
                            <th class="px-5 py-3 font-semibold text-right">Nominal Pembayaran</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        <tr>
                            <td colspan="8" class="px-5 py-10 text-center text-gray-400 dark:text-gray-500 text-sm">
                                Belum ada riwayat pembayaran piutang lama.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
