<x-app-layout>
    ...
    <x-slot name="header">
        <x-page-header icon="clipboard-list" title="Daftar Transaksi Lunas" subtitle="Daftar transaksi dan faktur yang telah terbayar lunas 100% (Sisa = Rp0)"></x-page-header>
    </x-slot>
    ...
    <div class="space-y-6 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="sw-card p-5 bg-gradient-to-br from-blue-50/80 to-white dark:from-blue-950/25 dark:to-gray-800 border-blue-100 dark:border-blue-900/40">
                <span class="text-[11px] font-bold uppercase tracking-wider text-blue-700 dark:text-blue-400">Total Transaksi Lunas</span>
                <p class="text-2xl sm:text-3xl font-extrabold font-mono text-blue-600 dark:text-blue-400 mt-2">
                    {{ $totalCount }} <span class="text-sm font-normal text-gray-400">transaksi lunas</span>
                </p>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Sisa tagihan = Rp0 (Lunas 100%)</p>
            </div>
            <div class="sw-card p-5">
                <span class="text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Total Nominal Terbayar</span>
                <p class="text-2xl sm:text-3xl font-bold font-mono text-green-600 dark:text-green-400 mt-2">
                    Rp {{ number_format($totalPaidAmount, 0, ',', '.') }}
                </p>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Total nilai transaksi yang telah diselesaikan</p>
            </div>
        </div>

        {{-- Filter & Search Box --}}
        <div class="sw-card p-4 sm:p-5">
            <form method="GET" action="{{ route('admin.receivables.summary.paid-transactions') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 items-end">
                <div>
                ...
                <div class="flex items-center pb-2">
                    <a href="{{ route('admin.receivables.summary.paid-transactions') }}" class="text-xs text-red-600 hover:underline font-semibold">Reset Filter</a>
                </div>
                @endif
            </form>
        </div>

        {{-- Table Transaksi Lunas --}}
        <div class="sw-card overflow-hidden">
            <div class="p-4 sm:p-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <div>
                    <h3 class="font-bold text-gray-900 dark:text-gray-100 text-sm sm:text-base">Daftar Faktur Lunas</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Menampilkan seluruh faktur yang telah selesai pembayarannya</p>
                </div>
