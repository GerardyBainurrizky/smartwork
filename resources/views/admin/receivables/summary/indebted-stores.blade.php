<x-app-layout>
    @section('breadcrumbs')
        ...
        <span>Daftar Toko Berpiutang</span>
    @endsection

    <x-slot name="header">
        <x-page-header icon="store" title="Daftar Toko Berpiutang" subtitle="Daftar toko aktif yang memiliki saldo tagihan berjalan (Sisa > Rp0)"></x-page-header>
    </x-slot>
    ...
    <div class="space-y-6 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="sw-card p-5">
                <span class="text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Total Toko Berpiutang</span>
                <p class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100 mt-2">
                    {{ $indebtedStores->count() }} <span class="text-sm font-normal text-gray-400">toko distinct</span>
                </p>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Saldo aktif &gt; Rp0</p>
            </div>
            <div class="sw-card p-5 bg-gradient-to-br from-red-50/80 to-white dark:from-red-950/25 dark:to-gray-800 border-red-100 dark:border-red-900/40">
                <span class="text-[11px] font-bold uppercase tracking-wider text-red-700 dark:text-red-400">Total Nilai Piutang</span>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Jumlah sisa saldo dari toko-toko ini</p>
            </div>
        </div>

        {{-- Filter & Search Box --}}
        <div class="sw-card p-4 sm:p-5">
            <form method="GET" action="{{ route('admin.receivables.summary.indebted-stores') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 items-end">
                <div>
                ...
                <div class="flex items-center pb-2">
                    <a href="{{ route('admin.receivables.summary.indebted-stores') }}" class="text-xs text-red-600 hover:underline font-semibold">Reset Filter</a>
                </div>
                @endif
            </form>
        </div>
        ...
        <div class="sw-card overflow-hidden">
            <div class="p-4 sm:p-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <div>
                    <h3 class="font-bold text-gray-900 dark:text-gray-100 text-sm sm:text-base">Daftar Toko Berpiutang</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Menampilkan seluruh toko dengan saldo tagihan belum lunas</p>
                </div>
                ...
                <table class="w-full text-left text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-800/60 text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">
                        <tr>
                            ...
                            <th class="px-5 py-3.5 font-semibold text-left">Kode Toko</th>
                            ...
                            <th class="px-5 py-3.5 font-semibold text-right">Total Piutang</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        <tr class="hover:bg-gray-50/60 dark:hover:bg-gray-800/40 transition">
                            <td class="px-5 py-4 text-left">
                                ...
                                @if($st->city)
                                <p class="text-xs text-gray-400 mt-0.5">{{ $st->city }}</p>
                                @endif
                            </td>
                            <td class="px-5 py-4 text-left font-mono text-xs text-gray-600 dark:text-gray-300">
                                {{ $st->code ?? '-' }}
                            </td>
                        ...
                        @empty
                        <tr>
                            <td colspan="7" class="px-5 py-10 text-center text-gray-400 dark:text-gray-500 text-sm">
                                Tidak ada toko dengan saldo piutang.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
